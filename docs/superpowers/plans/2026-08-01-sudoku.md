...
**Interfaces:**
- Consumes: `makeRng` from `@/app/game/_shared/rng`; `countSolutions`, `isValidGrid` from `solve.ts`; `hardestRank`, `tierOfRank` from `grade.ts`; `candidatesForGrid`, `digitsOf`, `countBits` from `candidates.ts`.
...
export function puzzleFor(tier: Tier, seed: number): Puzzle {
  ...
  let anyGraded: Puzzle | null = null;
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    // "tap the cell showing one mark" until it is solved. Hidden singles are
    // Easy's floor, which puts a rank-0 board below every tier, not merely
    // below the one that was asked for. Measured: the classic Wikipedia puzzle
    // is one of these, solving in 51 naked singles and no hidden single.
    if (rank === 0) continue;
  ...
  // Prefer a puzzle at the wrong tier over one with nothing to deduce, and
  // either over dealing no board at all.
  if (closest !== null) return closest;
  if (anyGraded !== null) return anyGraded;

  // Nothing graded in every attempt — vanishingly unlikely, but a game has to
  // deal a board rather than throw. Label it easy, since the techniques could
  // not prove otherwise.
  const rng = makeRng(seed);
  const solution = fullGrid(rng);
  return { givens: carve(solution, rng, "easy"), solution, tier: "easy", seed };
}
```
...
**Re-read `app/game/_shared/registry.ts` before editing it.** A concurrent session
rebuilt the arcade dashboard while this plan was being executed and added an
optional `progress?: ProgressSource` field to `ArcadeGame`, backed by a new
`app/game/_shared/progress.ts`. Match whatever shape the file actually has when
you get there; the snippet below is written against that rebuild.

Neither shared factory fits Sudoku. `levelProgress` reads `save.level` and
`waveProgress` reads `save.bestWave`; Sudoku's save has neither, keeping
per-tier stats instead. Supply the summary inline rather than adding a third
factory to `progress.ts` — that file is shared by every cabinet and another
session is actively editing it.
...
If the `progress` field no longer exists or has changed shape, omit it — two
cabinets (`chronicle`, `critter-knight`) already do, so a Sudoku entry without
it is valid and the game still lists and plays.
